...
#   -> "timeOfStartInDays" since the very first disaster in the dataset
...
def getIncidentTimesOfOccurenceColumn(df):
...
def getIncidentTimesOfEndingColumn(df):
    # get the min_date
    incidentBeginDates = pd.to_datetime(df["incidentBeginDate"])
    min_date = incidentBeginDates.min()

    # get the end dates (normalized)
    incidentEndDates = pd.to_datetime(df["incidentEndDate"])
    # Normalize the time axis (set the earliest date to 0)
    timeVals = (incidentEndDates - min_date).dt.total_seconds() / (24 * 3600)  # Convert to days
    return timeVals

def getIncidentDurationsColumn(df):
    timeVals = df["timeOfEndInDays"] - df["timeOfStartInDays"]
    return timeVals
...
def retrieveRowColFilteredDataset(inputDatasetPath = "Datasets/DisasterDeclarationsSummariesPlusLocation.csv",
    ...
    # NOTE: all possible disasterTypesOfInterest are ["fire", "tornado", "tsunami", "snow", "ice", "rain", "landslide", "flood", "blizzard", "wind", "thunderstorm", "storm", "earthquake", "hurricane", "temperature", "freez"]

    # first try to load the pre-filtered dataset (if exists)
    if enableLoadingSavedOutputFile and os.path.exists(outputFilteredDatafilePath):
    ...
    df["timeOfStartInDays"] = getIncidentTimesOfOccurenceColumn(df)
    df["timeOfEndInDays"] = getIncidentTimesOfEndingColumn(df)
    df["incidentDurationInDays"] = getIncidentDurationsColumn(df)
    df = df.sort_values(by="timeOfStartInDays")
